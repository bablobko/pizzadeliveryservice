 ...
 *
 */
public class NonVegPizzaDelvieryServiceImpl implements PizzaDeliveryService {
	...
	private void preparePizza(int pizzaQuantity) {
		while(pizzaQuantity > 0) {
			System.out.println("1 Non Veg Pizza got prepared.");
			pizzaQuantity--;
			try {
				Thread.sleep(5000);
			}catch(InterruptedException ie) {
		...
		System.out.println("All Non Veg pizzas got prepared.");
	}
